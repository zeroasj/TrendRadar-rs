...
每次触发（由 crond 或 systemd 定时）
  ...
  ├── 2. 抓取你订阅的 RSS 源（博客/新闻网站...，支持新鲜度过滤）
...
```
第 1 级：AI 管道格式分类（ID|TAG_ID|SCORE）→ 自然语言指令，模型直接输出
第 2 级：自动重试 3 次（指数退避 1s/2s/4s）→ 一个 chunk 失败不影响其他
第 3 级：关键词匹配兜底 → 100% 可靠，无需 API
```

> 每个 chunk 独立输出匹配结果。如果整个 chunk 没有一条新闻匹配任何标签，模型输出 `NONE`，程序直接跳过。日志会明确标注每个 chunk 的匹配数。
...
| 热榜平台 | 11 个 | 16 个（含华尔街见闻/澎湃/财联社/凤凰等）|
| 启动速度 | 几秒（解释型） | 毫秒级（编译型）|
...
rss:
  ...
  freshness_filter:
    ...
    max_age_days: 3       # 只保留 N 天内的文章，0 = 不过滤
  feeds:
    - name: "Hacker News"
      url: "https://hnrss.org/frontpage"
      enabled: true       # 单个源是否启用，默认 true
      max_age_days: 3     # 可选，覆盖全局设置
```
