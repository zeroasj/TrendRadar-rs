...
AI 分类内置两级降级保障，确保始终有结果输出：

```
第 1 级：pipe 格式解析（ID|TAG_ID|SCORE）→ AI 原生格式，极低出错率 ✅
第 2 级：frequency_words.txt 关键词匹配兜底 → 100% 可靠 ✅
```

> 第 1 级失败自动重试 3 次（指数退避 1s/2s/4s），一个 chunk 失败不影响其他 chunk。日志会明确标注「AI 分类成功」或「关键词兜底」。
