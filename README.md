...
找到 `notification` 这一段，`enabled` 改成 `true`，然后选一个渠道填上。**其他渠道留空就行，不会报错。**
...
MCP (Model Context Protocol) 可以让 AI 客户端（Claude Desktop / Cursor 等）直接查询你的热点数据库。

### 8.1 启动

```bash
./trendradar serve --host 0.0.0.0 --port 8080
```
...
|--------|------|
| `get_trends` | 获取最新热榜数据 |
| `get_trends_by_date` | 按日期查询历史热榜 |
| `get_rank_history` | 查询某条新闻的排名变化 |
| `search_news` | 按关键词搜索历史新闻 |
| `get_ai_analysis` | 获取 AI 分析结果 |
| `list_platforms` | 列出所有监控平台 |
| `get_status` | 查看服务运行状态 |
...
**A:** CPU 不支持编译的指令集。尝试用 `-C target-cpu=mips32r2` 重新编译。如果是软浮点问题，参考 [7.5 节](#75-常见特殊处理)。

### Q: 路由器上提示 "not found"

**A:** 动态链接器不匹配。检查并创建软链接，参考 [7.5 节](#75-常见特殊处理)。
...
| **小米 4A 路由器** (MIPS) | ~9 MB | ~12 MB | 10-30% |
...
敏感信息通过 `.env` 文件管理（由 `dotenvy` 加载），不写入 config.yaml，更安全：
